...
app.get('/veggies/', async (req, res) => {
    // res.send(fruits);
    try {
        const foundVeggies = await Veg.find({});
        res.status(200).render('Vegs/Index', {vegs: foundVeggies});
    } catch (err) {
        res.status(400).send(err);
    }
...
app.post('/veggies', async (req, res) => {
    ...
    try {
        const createdVeggies = await Vegs.create(req.body);
...
app.get('/veggies/:id', async (req, res) => {
    // res.send(fruits[req.params.indexOfFruitsArray]);
    try {
        const foundVeggies = await Vegs.findById(req.params.id);
        res.render('Vegs/Show', {Vegs: foundVeggies});
    } catch (err) {
        res.status(400).send(err);
    }
